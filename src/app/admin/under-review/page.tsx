...
/**
 * Suppliers the AI has already sent back at least once, and who have not
 * fixed it and resubmitted yet. Same shape as the supplier list, with the
 * AI's note added so you can see why, without opening each one.
 */
export default async function UnderReviewPage(
  props: PageProps<"/admin/under-review">,
) {
  ...
  let query = admin
    .from("profiles")
    .select(
      "id, full_name, company_name, email, phone, status, send_back_count, review_summary",
    )
    .eq("role", "supplier")
    .eq("status", "incomplete")
    .gte("send_back_count", 1)
    .order("send_back_count", { ascending: false });
  ...
  return (
    <>
      <div className="flex flex-wrap items-end justify-between gap-6">
        <div>
          <h1 className="adm-display text-[38px]">Under review</h1>
          <p className="mt-2 text-[14px] text-adm-mute">
            Sent back by the AI at least once, and still not fixed.{" "}
      ...
      <div className="mt-8 overflow-hidden rounded-[10px] border border-adm-line bg-adm-card">
        <table className="w-full text-left text-[15px]">
          <thead>
            <tr className="border-b border-adm-line text-[12px] uppercase tracking-wide text-adm-ash">
              ...
              <th className="px-5 py-3 font-normal">The AI&apos;s last note</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => (
              ...
              >
                ...
                <td className="max-w-[280px] px-5 py-4 text-[13px] text-adm-mute">
                  {row.review_summary ?? "—"}
                </td>
              </tr>
            ))}
        ...
        {rows.length === 0 && (
          <p className="px-5 py-10 text-center text-[15px] text-adm-mute">
            {search ? "Nobody matches that." : "Nobody is waiting to fix anything right now."}
          </p>
        )}
      </div>
    </>
